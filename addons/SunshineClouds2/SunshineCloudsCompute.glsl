...
void main() {
	...
	// GODOTWIND FIX: guard against division by zero when no clouds hit (lightingSamples=0).
	// Without this, paintedColor becomes NaN/0 → ambientLight killed → black disk at sun position.
	...
	// GODOTWIND FIX: Scale atmospheric contribution by cloud density via smoothstep.
	// Without this, the atmospheric scattering near the sun darkens the cloud color
	// in the temporal buffer, producing a dark disk on screen.
	float atmoCloudGateCompute = smoothstep(0.0, 0.3, density);
	if (linear_depth > maxstep && directionalLightCount > 0.0 && density > 0.001){
		for (float i = 0.0; i < directionalLightCount; i++){
			...
			// GODOTWIND FIX: max(0.05) prevents atmospheric blackout when looking at sun
			float sundensityaffect = max(0.05, 1.0 - clamp(dot(sundir, raydirection) * density, 0.0, 1.0));
			...
			physicalFogColor = mix(physicalFogColor, atmosphericData.rgb, atmosphericData.a * atmoCloudGateCompute); //causes jitter in the sky
			fogweight += atmosphericData.a * atmoCloudGateCompute;
		}
	}



	// GODOTWIND FIX: clamp fogweight to prevent undefined mix behavior when >1.0
