...
## Tests: SkyManager + SunshineClouds2 clouds + weather driving atmosphere params.
...
func _ready() -> void:
	...
	Log.info("weather", "Weather test scene ready. Keys: 1-0=weather, T/Y=time, W=toggle, K=Sky, O=ocean, F=fog, V=vol.fog")
...
func _process(delta: float) -> void:
	...
	# Update SkyManager with current game hour
...
func _toggle_sky() -> void:
	if _sky_manager == null:
		Log.warn("weather", "SkyManager not available")
		return
	...
	_sync_sky()
	Log.info("weather", "Sky: %s" % ("ON" if _sky_enabled else "OFF"))


func _sync_sky() -> void:
	if _sunshine_driver:
		if _sky_enabled and _sky_manager:
			var sun: DirectionalLight3D = _sky_manager.get_sun_light()
			if sun:
		...
		if clouds_res:
			...
			Log.info("weather", "SunshineClouds2 compositor re-registered after sky toggle")
...
func _update_debug_overlay() -> void:
	...
	# Clock
	lines.append("[b]Clock:[/b] Internal")
	...
	# Sky state
	lines.append("[b]Sky:[/b] %s" % ("ON" if _sky_enabled else "OFF"))
	if _sky_enabled and _sky_manager:
		lines.append("  Turbidity: %.1f" % _sky_manager.atmosphere.turbidity)
		lines.append("  Cloud coverage: %.2f" % _sky_manager.cloud_coverage)
	lines.append("")
