...
public partial class NativeNIFReader : RefCounted
{
	...
	private NIFRecord? ReadRecord(int index)
	{
		...
		NIFRecord? record = recordType switch
		{
			...
			"NiStringExtraData" or "NiExtraData" or
			...
			// Text Key Extra Data (animation markers)
			"NiTextKeyExtraData" => ReadNiTextKeyExtraData(),

			// Controllers with data reference
			"NiKeyframeController" or "NiVisController" or "NiAlphaController" or
			"NiMaterialColorController" or "NiRollController" or
			"NiLightColorController" or "NiLightDimmerController" or "NiLightRadiusController" or
			"NiTextureTransformController" or "NiTransformController"
				=> ReadNiControllerWithData(),

			// Controllers without data reference (just base NiTimeController)
			"NiBSPArrayController" or "NiMultiTargetTransformController" or
			"NiFloatExtraDataController" or "NiFloatsExtraDataController" or
			"NiFloatsExtraDataPoint3Controller" or "NiColorExtraDataController" or
			"NiBoolInterpController" or "NiPoint3InterpController" or "NiFloatInterpController"
				=> ReadNiTimeController(),

			// Special controllers with unique formats
			"NiFlipController" => ReadNiFlipController(),
			"NiUVController" => ReadNiUVController(),
			"NiPathController" => ReadNiPathController(),
			"NiLookAtController" => ReadNiLookAtController(),
			"NiParticleSystemController" => ReadNiParticleSystemController(),
			"NiGeomMorpherController" => ReadNiGeomMorpherController(),

			// Controller Data
			"NiKeyframeData" => ReadNiKeyframeData(),
			"NiFloatData" => ReadNiFloatData(),
			"NiPosData" => ReadNiPosData(),
			"NiColorData" => ReadNiColorData(),
			"NiVisData" => ReadNiVisData(),
			"NiUVData" => ReadNiUVData(),
			"NiMorphData" => ReadNiMorphData(),
	...
	private NIFRecord ReadNiTextKeyExtraData()
	{
		var extra = new NIFRecord();
		extra.ExtraDataIndex = ReadInt32();
		Skip(4); // unknown uint32

		uint numTextKeys = ReadUInt32();
		for (int i = 0; i < numTextKeys; i++)
		{
			Skip(4); // time (float)
			ReadString(); // text key string
		}
		return extra;
	}

	// Base controller reading - just NiTimeController fields
	private NIFRecord ReadNiTimeController()
	{
		var ctrl = new NIFRecord();
		ctrl.ControllerIndex = ReadInt32(); // next controller
		Skip(2); // flags
		Skip(4 * 4); // frequency + phase + start + stop
		Skip(4); // target index
		return ctrl;
	}

	// Controllers that have a data reference (NiKeyframeController, etc.)
	private NIFRecord ReadNiControllerWithData()
	{
		var ctrl = ReadNiTimeController();
		Skip(4); // data index
		return ctrl;
	}

	// NiFlipController: NiFloatInterpController + texSlot + timeStart + delta + sources list
	private NIFRecord ReadNiFlipController()
	{
		// NiFloatInterpController = NiTimeController for Morrowind (no interpolator)
		var ctrl = ReadNiTimeController();
		Skip(4); // texture slot (uint32)
		// Morrowind version <= 10.1.0.103 so these are present
		Skip(4); // start time (float)
		Skip(4); // delta (float)
		uint numSources = ReadUInt32();
		Skip((int)(numSources * 4)); // source texture refs
		return ctrl;
	}

	// NiUVController
	private NIFRecord ReadNiUVController()
	{
		var ctrl = ReadNiTimeController();
		Skip(2); // uv set index
		Skip(4); // data index
	...
	// NiPathController
	private NIFRecord ReadNiPathController()
	{
		var ctrl = ReadNiTimeController();
		// Morrowind doesn't have pathFlags field (only version >= 10.1.0.0)
		// mPathFlags = (mFlags >> 4) is calculated, not read
		Skip(4); // bank direction (int32)
		Skip(4); // max bank angle (float)
		Skip(4); // smoothing (float)
		Skip(2); // follow axis (short)
		Skip(4); // path data ref
		Skip(4); // percent data ref
		return ctrl;
	}

	// NiLookAtController
	private NIFRecord ReadNiLookAtController()
	{
		var ctrl = ReadNiTimeController();
		Skip(4); // look at target ref
		return ctrl;
	}

	// NiParticleSystemController (complex)
	private NIFRecord ReadNiParticleSystemController()
	{
		// Base NiTimeController
		var ctrl = new NIFRecord();
		ctrl.ControllerIndex = ReadInt32();
		Skip(2); // flags
		Skip(4 * 4); // frequency + phase + start + stop
		Skip(4); // target

		// Morrowind version >= 3.3.0.13, so all these fields are present
		Skip(4); // speed
		Skip(4); // speed variation
		Skip(4); // declination
		Skip(4); // declination variation
		Skip(4); // planar angle
		Skip(4); // planar angle variation
		Skip(12); // initial normal (Vec3)
		Skip(16); // initial color (Color4)
		Skip(4); // initial size
		Skip(4); // emit start time
		Skip(4); // emit stop time
		Skip(1); // reset particle system (byte)
		Skip(4); // birth rate
		Skip(4); // lifetime
		Skip(4); // lifetime variation
		Skip(2); // emit flags (ushort)
		Skip(12); // emitter dimensions (Vec3)
		Skip(4); // emitter ref

		Skip(2); // num spawn generations (ushort)
		Skip(4); // percentage spawned (float)
		Skip(2); // spawn multiplier (ushort)
		Skip(4); // spawn speed chaos
		Skip(4); // spawn dir chaos

		ushort numParticles = ReadUInt16();
		Skip(2); // num valid (ushort)

		// Skip particle info array (NiParticleInfo)
		for (int i = 0; i < numParticles; i++)
		{
			Skip(12); // velocity (Vec3)
			Skip(12); // rotation axis (Vec3) - Morrowind version
			Skip(4); // age
			Skip(4); // lifespan
			Skip(4); // last update
			Skip(2); // spawn generation (ushort)
			Skip(2); // code (ushort)
		}

		Skip(4); // emitter modifier ref (skipped in OpenMW)
		Skip(4); // modifier ref
		Skip(4); // collider ref
		Skip(1); // static target bound (uint8)

		return ctrl;
	}

	// NiGeomMorpherController
	private NIFRecord ReadNiGeomMorpherController()
	{
		var ctrl = ReadNiTimeController();
		Skip(4); // data ref
		Skip(1); // always active
		return ctrl;
	}

	private NiKeyframeData ReadNiKeyframeData()
	{
		var data = new NiKeyframeData();

		// Rotation keys
		uint numRotKeys;
		data.RotationKeyType = ReadKeyMap(KeyValueType.Quaternion, out numRotKeys);
		data.NumRotationKeys = numRotKeys;

		// If XYZ rotation type, read 3 separate float key maps
		if (data.RotationKeyType == KeyType.XYZ_ROTATION)
		{
			// Axis order (Morrowind version)
			if (_version <= VER_MW)
				data.AxisOrder = ReadUInt32();

			// X, Y, Z rotation keys
			ReadKeyMap(KeyValueType.Float, out _);
			ReadKeyMap(KeyValueType.Float, out _);
			ReadKeyMap(KeyValueType.Float, out _);
		}

		// Translation keys
		ReadKeyMap(KeyValueType.Vector3, out _);
	...
	private NIFRecord ReadNiFloatData()
	{
	...
	private NIFRecord ReadNiPosData()
	{
		var data = new NIFRecord();
		ReadKeyMap(KeyValueType.Vector3, out _);
		return data;
	...
	private NIFRecord ReadNiColorData()
	{
		var data = new NIFRecord();
		ReadKeyMap(KeyValueType.Color4, out _);
		return data;
	}

	private NIFRecord ReadNiVisData()
	{
		var data = new NIFRecord();
		uint numKeys = ReadUInt32();
		for (int i = 0; i < numKeys; i++)
		{
			Skip(4); // time (float)
			Skip(1); // visibility (byte/bool)
		}
		return data;
	}

	private NIFRecord ReadNiUVData()
	{
		var data = new NIFRecord();
		// 4 float key maps: U offset, V offset, U scale, V scale
		for (int i = 0; i < 4; i++)
		{
			ReadKeyMap(KeyValueType.Float, out _);
		}
	...
	private NIFRecord ReadNiMorphData()
	{
		var data = new NIFRecord();
		uint numMorphs = ReadUInt32();
		uint numVertices = ReadUInt32();
		Skip(1); // relative targets (byte)

		for (int i = 0; i < numMorphs; i++)
		{
			// Each morph has a float key map for weights
			ReadKeyMap(KeyValueType.Float, out _, isMorph: true);
			// Then vertex offsets
			Skip((int)(numVertices * 12)); // Vector3 per vertex
		}
		return data;
	}

	// Key interpolation types
	public enum KeyType : uint
	{
		LINEAR = 1,
		QUADRATIC = 2,
		TBC = 3,  // Tension-Continuity-Bias
		XYZ_ROTATION = 4,
		CONSTANT = 5
	}
	...
	private enum KeyValueType
	{
		Float,      // 4 bytes
		Vector3,    // 12 bytes
		Quaternion, // 16 bytes (w, x, y, z)
		Color4      // 16 bytes
	}

	/// <summary>
	/// Read a key map (animation key data) and return the interpolation type.
	/// </summary>
	private KeyType ReadKeyMap(KeyValueType valueType, out uint numKeys, bool isMorph = false)
	{
		numKeys = ReadUInt32();

		if (numKeys == 0 && !isMorph)
			return KeyType.LINEAR;

		KeyType keyType = (KeyType)ReadUInt32();

		// Calculate value size based on type
		int valueSize = valueType switch
		{
			KeyValueType.Float => 4,
			KeyValueType.Vector3 => 12,
			KeyValueType.Quaternion => 16,
			KeyValueType.Color4 => 16,
			_ => 4
		};
		...
		switch (keyType)
		{
			...
			case KeyType.CONSTANT:
				// time + value
				Skip((int)(numKeys * (4 + valueSize)));
				break;

			case KeyType.QUADRATIC:
				if (valueType == KeyValueType.Quaternion)
				{
					// Quaternions don't have tangents in quadratic mode
					Skip((int)(numKeys * (4 + valueSize)));
				}
				else
				{
					// time + value + inTangent + outTangent
					Skip((int)(numKeys * (4 + valueSize * 3)));
				}
				break;

			case KeyType.TBC:
				// time + value + tension + continuity + bias
				Skip((int)(numKeys * (4 + valueSize + 12)));
				break;

			case KeyType.XYZ_ROTATION:
				// XYZ rotation is handled separately - no data here
				break;

			default:
				if (numKeys > 0 && DebugMode)
					GD.PushWarning($"NIFReader: Unknown key type {(uint)keyType}");
				break;
		}
	...
	private string ReadString()
	{
		...
		if (length == 0 || length > 65535 || _pos + length > _buffer.Length)
		{
			...
			{
				string pathInfo = string.IsNullOrEmpty(_sourcePath) ? "" : $" in '{_sourcePath}'";
				GD.PushError($"NIFReader: Invalid string length {length} at pos {_pos}{pathInfo}");
				_parseFailed = true;
			}
			return "";
		}
...
public partial class NiKeyframeData : NIFRecord
{
	public uint NumRotationKeys { get; set; }
	public uint AxisOrder { get; set; }
	// Store the internal key type as uint for external access
	internal NativeNIFReader.KeyType RotationKeyType { get; set; }
}
