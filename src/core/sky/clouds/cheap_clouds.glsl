...
// (MIT, see assets/sky/ATTRIBUTION.txt). Raymarches the cloud hemisphere into
// an octahedral-mapped texture, amortized over N frames by the driver
// (cheap_cloud_layer.gd). The ONLY structural change from upstream: the demo's
// Hillaire sky-view LUT (set 2 sampler) is replaced with three precomputed
// atmosphere colors in a uniform buffer — Godotwind's sky is analytical and
// has no LUT. Everything else is kept 1:1 to stay diffable against upstream.
...
// Replaces the demo's sky_lut sampler: sky radiance toward the sun, at ~45°
// altitude (ambient), and toward the ground, precomputed on the CPU per
// texture swap. Magnitudes are calibrated to the demo's LUT scale (~50x
// display radiance) so the downstream constants stay untouched.
layout(set = 2, binding = 0, std140) uniform AtmoColors {
	vec4 sun;
	vec4 ambient;
	vec4 ground;
} atmo;
...
vec4 march(vec3 pos,  vec3 end, vec3 dir, int depth) {
	...
	// Sun and ambient colors from the precomputed atmosphere buffer
	// (upstream sampled its sky-view LUT here).
	vec3 atmosphere_sun = atmo.sun.rgb * 0.1 * params.LIGHT_ENERGY * params.LIGHT_COLOR;
	vec3 atmosphere_ambient = atmo.ambient.rgb * 0.05;
	...
	vec3 atmosphere_ground = atmo.ground.rgb * 5.0 * 0.05;
