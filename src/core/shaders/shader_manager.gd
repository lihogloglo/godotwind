...
func _ready() -> void:
	...
	_load_config()
	print("[ShaderManager] Initialized with %d effects" % _effects.size())
...
func attach_to(node: Node) -> void:
	...
	if node is WorldEnvironment:
		...
		print("[ShaderManager] Attached to WorldEnvironment")
	elif node is Camera3D:
		node.compositor = _compositor
		print("[ShaderManager] Attached to Camera3D")
	else:
...
func load_effect(path: String) -> bool:
	...
	print("[ShaderManager] Loaded effect: %s" % effect.effect_name)
	return true
...
func unload_effect(effect_name: String) -> void:
	...
	print("[ShaderManager] Unloaded effect: %s" % effect_name)
...
func enable_effect(effect_name: String, transition_time: float = 0.0) -> bool:
	...
	print("[ShaderManager] Enabled effect: %s" % effect_name)
	return true
...
func _remove_effect_from_compositor(effect_name: String) -> void:
	...
	stack_changed.emit(_effect_stack.duplicate())
	print("[ShaderManager] Disabled effect: %s" % effect_name)
...
func save_preset(preset_name: String) -> void:
	...
	print("[ShaderManager] Saved preset: %s" % preset_name)
...
func load_preset(preset_name: String) -> bool:
	...
	_current_preset = preset_name
	print("[ShaderManager] Loaded preset: %s" % preset_name)
	return true
...
func save_config() -> void:
	...
	if err != OK:
		...
		print("[ShaderManager] Config saved to %s" % CONFIG_PATH)
...
func _load_config() -> void:
	...
	print("[ShaderManager] Config loaded from %s" % CONFIG_PATH)
