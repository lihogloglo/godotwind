	...
	for path: String in YAML_SEARCH_PATHS:
		if FileAccess.file_exists(path):
			if load_from_file(path):
				if debug_mode:
					Log.debug("collision", "Auto-loaded from %s" % path)
				return
...
func _parse_yaml(content: String) -> bool:
	...
	if debug_mode:
		Log.debug("collision", "Loaded %d items, %d patterns" % [
...
func get_shape_for_item(item_id: String) -> Variant:  ## Returns ShapeType or null
	...
	if lower_id in _item_shapes:
		if debug_mode:
			Log.debug("collision", "Exact match for '%s' -> %s" % [
	...
	for compiled: CompiledPattern in _patterns:
		if _pattern_matches(compiled, lower_id):
			if debug_mode:
				Log.debug("collision", "Pattern '%s' matched '%s' -> %s" % [
