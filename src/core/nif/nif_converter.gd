...
func _convert() -> Node3D:
	...
	if not _is_native_reader and _reader != null:
		...
		if auto_collision_mode and not _source_path.is_empty():
			...
			if debug_collision:
				Log.debug("nif", "NIFConverter: Auto-selected collision mode %d for %s" % [
	...
	if has_skinning:
		...
		if skeleton:
			...
			if debug_skinning:
				Log.debug("nif", "NIFConverter: Created skeleton with %d bones" % skeleton.get_bone_count())
	...
	if load_animations and _has_animation_data():
		...
		if anim_player:
			...
			if debug_animations:
				Log.debug("nif", "NIFConverter: Created AnimationPlayer with %d animations" % anim_player.get_animation_library("").get_animation_list().size())
	...
	if load_collision:
		...
		if collision_result.has_collision:
			...
			if static_body:
				...
				if debug_collision:
					Log.debug("nif", "NIFConverter: Created StaticBody3D with %d collision shapes" % collision_result.collision_shapes.size())
...
func _add_occluder_to_mesh(mesh_instance: MeshInstance3D) -> void:
	...
	if parent:
		...
		if debug_occluders:
			Log.debug("nif", "NIFConverter: Added occluder to '%s' (size: %v)" % [
...
func _link_skinned_meshes_to_skeleton(node: Node, skeleton: Skeleton3D) -> void:
	if node is MeshInstance3D:
		...
		if mesh_instance.has_meta("_has_skeleton"):
			...
			if debug_skinning:
				Log.debug("nif", "NIFConverter: Linked '%s' to skeleton (path=%s)" % [
...
func create_skeleton_from_hierarchy() -> Skeleton3D:
	...
	if debug_skinning:
		Log.debug("nif", "NIFConverter: Building skeleton from hierarchy, root='%s'" % root_node.name)
...
func _convert_ni_tri_shape(shape: Defs.NiTriShape, skeleton: Skeleton3D = null) -> MeshInstance3D:
	...
	if has_skin_data:
		...
		if debug_skinning:
			Log.debug("nif", "NIFConverter: Converting skinned mesh '%s'" % mesh_instance.name)
...
func _convert_ni_tri_strips(strips: Defs.NiTriStrips, skeleton: Skeleton3D = null) -> MeshInstance3D:
	...
	if has_skin_data:
		...
		if debug_skinning:
			Log.debug("nif", "NIFConverter: Converting skinned strip mesh '%s'" % mesh_instance.name)
...
func _create_skinned_tri_shape_mesh(data: Defs.NiTriShapeData, skin_instance: Defs.NiSkinInstance, skin_data: Defs.NiSkinData) -> ArrayMesh:
	...
	if not skin_arrays.is_empty():
		...
		if indices_arr and weights_arr and indices_arr.size() > 0:
			...
			if debug_skinning:
				Log.debug("nif", "  Added bone weights: %d vertices, %d indices, %d weights" % [
					data.num_vertices,
					indices_arr.size(),
					weights_arr.size()
				])
	...
	for anim_name: String in animations:
		...
		if anim:
			...
			if err != OK:
				...
				Log.debug("nif", "  Added animation '%s' (%.2fs, %d tracks)" % [
...
func _convert_merged() -> Node3D:
	...
	if debug_merge:
		...
		Log.debug("nif", "NIFConverter: Merging %d shapes into %d material groups for '%s'" % [
	...
	for material_key: String in collected:
		...
		if material == null and not all_hidden:
			Log.debug("nif", "NIFConverter: Hiding materialless mesh in %s (likely collision geometry)" % _source_path.get_file())
		...
		if debug_merge:
			...
			Log.debug("nif", "  Material group '%s': %d shapes merged, %d triangles" % [
