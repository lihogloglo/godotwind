...
func _init() -> void:
	...
	priority = 3  # Highest ground locomotion priority
...
func _apply_movement(input: InputPackage, delta: float) -> void:
	...
	if absf(angle) >= config.ground_tracking_angular_speed * delta:
		move_dir = face_direction.rotated(
			Vector3.UP, signf(angle) * config.ground_tracking_angular_speed * delta) * config.sprint_turn_speed
		facing.rotate_y(signf(angle) * config.ground_tracking_angular_speed * delta)
	else:
		move_dir = face_direction.rotated(Vector3.UP, angle) * config.sprint_speed
	...
	var vel_ratio := h_speed / maxf(config.sprint_speed, 0.001)
