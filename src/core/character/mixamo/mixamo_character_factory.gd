...
#   # Full NPC with animation system:
#   var npc := factory.create_npc("res://character.fbx", ["res://walk.fbx", "res://idle.fbx"])
#   # Or low-level:
#   var character := factory.create_character("res://character.fbx")
...
## Animation cache: fbx_path -> AnimationLibrary
...
func _init() -> void:
	...
	Log.info("character", "MixamoCharacterFactory initialized")

## Load Mixamo character from FBX file
...
func create_character(fbx_path: String) -> Node3D:
	if not FileAccess.file_exists(fbx_path):
		Log.error("character", "FBX file not found: %s" % fbx_path)
		return null

	Log.info("character", "Loading Mixamo character: %s" % fbx_path)

	# Load FBX scene
	var scene: PackedScene = load(fbx_path)
	if scene == null:
		Log.error("character", "Failed to load FBX as PackedScene: %s" % fbx_path)
		return null

	var imported_root := scene.instantiate()
	if imported_root == null:
		Log.error("character", "Failed to instantiate FBX scene: %s" % fbx_path)
		return null
	...
	if skeleton == null:
		Log.error("character", "No Skeleton3D found in FBX: %s" % fbx_path)
		imported_root.queue_free()
		return null

	if mesh_instance == null:
		Log.warn("character", "No MeshInstance3D found in FBX: %s" % fbx_path)
	...
	Log.info("character", "Mixamo character created: %d bones" % skeleton.get_bone_count())
	return character_root

## Create a full NPC from Mixamo FBX with animation system, collision, and AI movement.
## Returns CharacterMovementController matching CharacterFactoryV2.create_npc() output format.
##
## animation_paths: FBX files to load animations from (merged into single library).
##   Include the character FBX itself if it contains animations you want.
...
##       ├── AnimationPlayer (with loaded FBX animations)
##       └── HumanoidAnimationSystem (Node)
func create_npc(fbx_path: String, animation_paths: Array = [],
		height: float = 1.8, radius: float = 0.4) -> CharacterBody3D:
	var start := Time.get_ticks_msec()

	# 1. Create character root with skeleton + mesh
	var character_root := create_character(fbx_path)
	...
	# 4. Load and merge all animation FBXs into a single library
	...
	movement_controller.name = fbx_path.get_file().get_basename()
	...
	movement_controller.set_meta("character_type", "MIXAMO")
	movement_controller.set_meta("is_character", true)
	movement_controller.set_meta("uses_new_animation_system", true)

	Log.info("character", "Mixamo NPC created: %s (%d ms)" % [
		fbx_path.get_file(), Time.get_ticks_msec() - start])
	return movement_controller


## Load animation library from FBX/GLB file
## Animations must target the same skeleton bone structure
## preserve_bone_names: if true, keeps original Mixamo bone names (default: true)
...
func load_animation_library(anim_path: String, skeleton: Skeleton3D, preserve_bone_names: bool = true) -> AnimationLibrary:
	...
	Log.info("character", "Loading Mixamo animations: %s" % anim_path)

	# Use AnimationLoader with preserve_bone_names flag
	var library := AnimationLoader.load_from_fbx(anim_path, {}, preserve_bone_names)
	if not library:
...
func _setup_animation_system(character_root: Node3D, skeleton: Skeleton3D) -> void:
	...
	# NOTE: root_node defaults to "." (self), which means animation tracks
	# use paths relative to CharacterRoot (AnimationPlayer's parent).
	# Animation tracks will be in format "Skeleton:bonename" to access bones.
...
func _apply_skeleton_profile(skeleton: Skeleton3D) -> void:
	# Map Mixamo bone names to SkeletonProfileHumanoid
	# This enables IK to work without retargeting animations
	var bone_map := _create_mixamo_bone_map(skeleton)
	if bone_map:
		# Store for IK setup (profile is used by TwoBoneIK3D, not for animation)
...
## INTERNAL: Create bone map for Mixamo -> HumanoidProfile
## Returns BoneMap or null if mapping fails
func _create_mixamo_bone_map(skeleton: Skeleton3D) -> BoneMap:
	var bone_map := BoneMap.new()
	bone_map.profile = _skeleton_profile

	# Detect Mixamo prefix (mixamorig1_, mixamorig_, mixamorig:, or no prefix)
	var mixamo_prefix := _detect_mixamo_prefix(skeleton)
	Log.info("character", "Detected Mixamo prefix: '%s'" % mixamo_prefix)

	# Common Mixamo -> Humanoid mappings (unprefixed names)
	var mappings := {
		"Hips": "Hips",
		"Spine": "Spine",
		"Spine1": "Chest",
	...
	var mapped_count := 0
	for mixamo_name in mappings:
		var profile_name: String = mappings[mixamo_name]
		var full_bone_name: String = mixamo_prefix + mixamo_name
		if skeleton.find_bone(full_bone_name) != -1:
			bone_map.set_skeleton_bone_name(profile_name, full_bone_name)
			mapped_count += 1
			Log.debug("character", "  Mapped: %s -> %s" % [profile_name, full_bone_name])

	if mapped_count == 0:
		Log.warn("character", "No Mixamo bones mapped to profile")
		return null

	Log.info("character", "Mapped %d bones to profile" % mapped_count)
	return bone_map

## INTERNAL: Detect Mixamo bone name prefix
## Returns the prefix string (e.g., "mixamorig1_", "mixamorig:", "")
func _detect_mixamo_prefix(skeleton: Skeleton3D) -> String:
	if skeleton.get_bone_count() == 0:
		return ""

	# Check first bone name for common Mixamo prefixes
	var first_bone := skeleton.get_bone_name(0)

	# Try common prefixes
	var prefixes := ["mixamorig1_", "mixamorig_", "mixamorig:", "Mixamorig_", "Mixamorig:"]
	for prefix in prefixes:
		if first_bone.begins_with(prefix):
			return prefix

	# No prefix (unprefixed Mixamo or custom skeleton)
	return ""
