...
func initialize(size: int = 256) -> void:
	...
	if rd == null:
		Log.info("water", "WaveGenerator: No RenderingDevice available - using flat plane fallback")
		_use_compute = false
		return
	...
	if not FileAccess.file_exists(shader_path):
		Log.info("water", "WaveGenerator: Compute shaders not found - using flat plane fallback")
		_use_compute = false
		return

	_use_compute = true
	Log.info("water", "WaveGenerator: GPU compute shaders available, map size: %d" % map_size)
...
func init_gpu(num_cascades: int) -> void:
	...
	_initialized = true
	Log.info("water", "WaveGenerator: GPU compute initialized with %d cascades" % num_cascades)
