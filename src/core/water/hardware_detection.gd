...
static func detect() -> void:
	...
	Log.info("water", "HardwareDetection: GPU: %s" % _renderer_name)
	...
	Log.info("water", "HardwareDetection: Integrated GPU: %s" % _is_integrated_gpu)
	Log.info("water", "HardwareDetection: Has compute: %s" % _has_compute)
	Log.info("water", "HardwareDetection: Recommended water quality: %s" % WaterQuality.keys()[_recommended_quality])
