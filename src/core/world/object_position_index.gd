...
func build_from_esm() -> void:
	...
	Log.info("streaming", "ObjectPositionIndex: Built in %.1fms - %d objects in %d cells" % [
...
func save_to_file(path: String) -> Error:
	...
	Log.info("streaming", "ObjectPositionIndex: Saved to %s (%.1f KB)" % [path, json.length() / 1024.0])
	return OK
...
func load_from_file(path: String) -> Error:
	...
	Log.info("streaming", "ObjectPositionIndex: Loaded from %s in %.1fms - %d objects" % [
