...
func update_chunks(camera_cell: Vector2i) -> void:
	...
	_update_tier_chunks(
		...
		visible_by_tier.get(DistanceTierManagerScript.Tier.MID, []),
		camera_cell
	)
	...
	_update_tier_chunks(
		...
		visible_by_tier.get(DistanceTierManagerScript.Tier.FAR, []),
		camera_cell
	)
...
func _load_mid_chunk(chunk_grid: Vector2i, identifier: String) -> void:
	...
	var cells: Array[Vector2i] = chunk_manager.get_cells_in_chunk(chunk_grid, QuadtreeChunkManagerScript.MID_CHUNK_SIZE)
		...
		if ResourceLoader.exists(prebaked_path):
			...
			if mesh:
				if distant_renderer.has_method("add_cell_prebaked"):
					distant_renderer.add_cell_prebaked(cell_grid, mesh)
	...
	if distant_renderer:
		for cell_grid in chunk_data.cells_loaded:
			if distant_renderer.has_method("remove_cell"):
				distant_renderer.remove_cell(cell_grid)
	...
	for cell_grid in cells:
		...
		if cell_record:
			if impostor_manager.has_method("add_cell_impostors"):
				var count: int = impostor_manager.add_cell_impostors(cell_grid, cell_record.references)
				if count > 0:
					chunk_data.cells_loaded.append(cell_grid)
	...
	if impostor_manager:
		for cell_grid in chunk_data.cells_loaded:
			if impostor_manager.has_method("remove_impostors_for_cell"):
				impostor_manager.remove_impostors_for_cell(cell_grid)
...
func is_chunk_loaded(chunk_grid: Vector2i, tier: int) -> bool:
	var identifier: String = chunk_manager.get_chunk_identifier(chunk_grid, tier) if chunk_manager else ""
	match tier:
		DistanceTierManagerScript.Tier.MID:
...
func clear() -> void:
	...
	for identifier in _loaded_mid_chunks.keys():
		_unload_mid_chunk(identifier)

	# Unload all FAR chunks
	for identifier in _loaded_far_chunks.keys():
		_unload_far_chunk(identifier)
