## MorrowindTerrainManager - Converts Morrowind LAND data to Terrain3D format
...
class_name MorrowindTerrainManager
extends "res://src/core/world/terrain_manager.gd"
...
func _check_native_availability() -> void:
	...
	if _use_native:
		Log.info("streaming", "MorrowindTerrainManager: Using native C# terrain generation (50-100x faster)")
...
# External source texture slot mapper
...
func generate_heightmap(land: LandRecord) -> Image:
	if not land.has_heights():
		push_warning("MorrowindTerrainManager: LAND record %d,%d has no height data" % [land.cell_x, land.cell_y])
...
func generate_control_map(land: LandRecord) -> Image:
	...
	if not land.has_textures():
		...
		if _debug_control_map:
			Log.debug("streaming", "MorrowindTerrainManager: Cell (%d,%d) has NO texture data - using default" % [land.cell_x, land.cell_y])
		return img
	...
	if _stats["control_maps_generated"] == 0:
		...
		Log.info("streaming", "MorrowindTerrainManager: Using control map mode %d (%s)" % [control_map_mode, mode_name])
...
func _generate_control_map_terrain3d_optimized(img: Image, land: LandRecord, vertices_per_tex: float) -> void:
	...
	if _debug_control_map or _stats["control_maps_generated"] < 3:
		...
		Log.debug("streaming", "MorrowindTerrainManager: Cell (%d,%d) blend stats: %d/%d pixels blended (%.1f%%), max=%d, avg=%.1f" % [
...
func debug_print_control_map_stats(land: LandRecord) -> void:
	if not land.has_textures():
		Log.debug("streaming", "MorrowindTerrainManager: Cell (%d,%d) has NO texture data" % [land.cell_x, land.cell_y])
		return
	...
	Log.debug("streaming", "MorrowindTerrainManager: Cell (%d,%d) MW->Slot mappings: %s" % [land.cell_x, land.cell_y, mw_indices])
	Log.debug("streaming", "MorrowindTerrainManager: Cell (%d,%d) slot usage counts: %s" % [land.cell_x, land.cell_y, tex_usage])
...
## Set the texture slot mapper supplied by the active terrain source.
...
func _get_terrain3d_texture_slot(mw_tex_idx: int) -> int:
	...
	if _texture_slot_mapper and _texture_slot_mapper.has_method("get_slot_for_mw_index"):
		...
		if _debug_control_map and _stats["control_maps_generated"] < 3:
			Log.debug("streaming", "MorrowindTerrainManager: _get_terrain3d_texture_slot(%d) -> slot %d (via mapper)" % [mw_tex_idx, slot])
		return slot
	...
	if _debug_control_map:
		push_warning("MorrowindTerrainManager: Using fallback mapping for MW index %d -> slot %d (no mapper!)" % [mw_tex_idx, slot])
	return slot
...
func import_cell_to_terrain(terrain: Terrain3D, land: LandRecord, local_coord: Vector2i = Vector2i.ZERO, use_local_coord: bool = false) -> bool:
	...
	if use_local_coord and (cell_x == 0 and cell_y == 0) or (cell_x == -8 and cell_y == -8):
		Log.debug("streaming", "[MorrowindTerrainManager] Importing cell (%d,%d) at world pos: (%.1f, %.1f, %.1f)" % [
			cell_x, cell_y, import_pos.x, import_pos.y, import_pos.z
		])
		Log.debug("streaming", "  Terrain global_position: (%.1f, %.1f, %.1f)" % [
			terrain.global_position.x, terrain.global_position.y, terrain.global_position.z
		])
		Log.debug("streaming", "  use_local_coord: %s, region_world_size: %.2f" % [use_local_coord, region_world_size])
