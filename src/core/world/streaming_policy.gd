...
## - `is_mid_worthy()` — runtime: decides if object belongs in MID/HLOD source data. Post-S.1
...
## Check if a model is visually significant enough for MID/HLOD renderer ownership.
