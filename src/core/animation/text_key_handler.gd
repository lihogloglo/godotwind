...
func register_text_keys(animation_name: String, text_keys: Array) -> void:
	...
	for key: Dictionary in text_keys:
		...
		if debug_mode:
			Log.debug("animation", "TextKeyHandler: Registered key '%s' at %.3fs for '%s'" % [
...
func _trigger_key(key: Dictionary, position: Vector3) -> void:
	...
	if debug_mode:
		Log.debug("animation", "TextKeyHandler: Triggering '%s' at %.3fs (type=%d)" % [raw, time, key_type])
...
func _on_animation_started(anim_name: StringName) -> void:
	...
	if debug_mode:
		Log.debug("animation", "TextKeyHandler: Animation started '%s'" % anim_name)
...
func _on_animation_finished(anim_name: StringName) -> void:
	...
	if debug_mode:
		Log.debug("animation", "TextKeyHandler: Animation finished '%s'" % anim_name)
