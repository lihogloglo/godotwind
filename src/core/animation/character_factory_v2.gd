...
var body_part_assembler: BodyPartAssembler = null
...
func create_npc(npc_record: NPCRecord, ref_num: int = 0) -> CharacterBody3D:
	...
	# Load animations
	_load_character_animations(character_root, is_female, is_beast)
	...
	if debug_characters:
		print("CharacterFactoryV2: Created NPC '%s' (%s, %s)" % [
...
func create_creature(creature_record: CreatureRecord, ref_num: int = 0) -> CharacterBody3D:
	...
	if not character_root:
		push_warning("CharacterFactoryV2: Failed to assemble creature '%s'" % creature_record.record_id)
...
func _load_character_animations(character_root: Node3D, is_female: bool, is_beast: bool) -> void:
	...
	if not skeleton:
		if debug_characters:
			print("CharacterFactoryV2: No Skeleton3D found in character root")
		return
	...
	if animations.is_empty():
		if debug_characters:
			print("CharacterFactoryV2: No animations extracted from '%s'" % full_path)
		return
	...
	if debug_characters:
		print("CharacterFactoryV2: Loaded %d animations from '%s'" % [added_count, anim_path])
