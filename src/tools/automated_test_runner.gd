...
func print_report() -> void:
	...
	Log.info("testing", "")
	Log.info("testing", "╔════════════════════════════════════════════════════════════════╗")
	Log.info("testing", "║               AUTOMATED TEST REPORT                            ║")
	Log.info("testing", "╠════════════════════════════════════════════════════════════════╣")
	...
	Log.info("testing", "║ Duration: %.1f seconds" % duration_val)
	Log.info("testing", "║ Waypoints: %d/%d completed" % [wp_completed, wp_total])
	...
	Log.info("testing", "║   Avg FPS: %.1f" % avg_fps)
	Log.info("testing", "║   Frame time: %.2fms avg, %.2fms P95, %.2fms max" % [avg_ft, p95_ft, max_ft])
	...
	if not times.is_empty():
		Log.info("testing", "╠════════════════════════════════════════════════════════════════╣")
		Log.info("testing", "║ LOADING TIMES (by waypoint)")
		for wp_name: String in times.keys():
			var time_val: float = times[wp_name] as float
			Log.info("testing", "║   %s: %.2fs" % [wp_name, time_val])
	...
	Log.info("testing", "║ ERRORS: %d total (%d unique)" % [err_total, err_unique])
	...
	for key: String in error_groups_dict.keys():
		...
		Log.info("testing", "║   [x%d] %s" % [cnt, key.substr(0, 60)])
	...
	Log.info("testing", "║ WARNINGS: %d total (%d unique)" % [warn_total, warn_unique])
	var warning_groups_dict: Dictionary = warnings.get("groups", {})
	var shown: int = 0
	for key: String in warning_groups_dict.keys():
		if shown >= 10:
			Log.info("testing", "║   ... and %d more warning types" % (warning_groups_dict.size() - shown))
			break
