...
func _run_import() -> void:
	...
	Log.info("tools", "=".repeat(60))
	Log.info("tools", "La Palma Terrain Import (Memory-Optimized)")
	Log.info("tools", "=".repeat(60))
	...
	Log.info("tools", "World: %s" % _metadata.get("world_name", "Unknown"))
	Log.info("tools", "Region size: %d pixels" % region_size)
	Log.info("tools", "Vertex spacing: %.1fm" % vs)
	Log.info("tools", "Batch size: %d regions" % batch_size)
	...
	Log.info("tools", "Regions to import: %d (of %d total)" % [total, regions.size()])
	...
	for i in range(total):
		...
		if imported % 10 == 0 or imported == total:
			...
			Log.info("tools", "  [%d/%d] %.1f/sec, ~%.0fs remaining" % [imported, total, rate, remaining])

	var elapsed := (Time.get_ticks_msec() - start_time) / 1000.0

	Log.info("tools", "=".repeat(60))
	Log.info("tools", "Import Complete!")
	Log.info("tools", "=".repeat(60))
	Log.info("tools", "  Regions imported: %d" % imported)
	Log.info("tools", "  Time: %.1f seconds" % elapsed)
	Log.info("tools", "Now click 'Save Terrain' to save to disk.")
	Log.info("tools", "=".repeat(60))
...
func _save_terrain() -> void:
	...
	Log.info("tools", "Saving terrain to: " + global_path)
	...
	Log.info("tools", "=".repeat(60))
	Log.info("tools", "Terrain Saved!")
	Log.info("tools", "=".repeat(60))
	Log.info("tools", "  Output: %s" % OUTPUT_DIR)
	Log.info("tools", "  Size: %.1f MB" % (total_size / 1024.0 / 1024.0))
	Log.info("tools", "  Regions: %d" % data.get_region_count())
	Log.info("tools", "You can now delete:")
	Log.info("tools", "  - lapalma_processed/  (~900 MB)")
	Log.info("tools", "  - lapalma_map/        (~2.2 GB)")
	Log.info("tools", "=".repeat(60))
...
func _clear_terrain() -> void:
	if not data:
		return
	Log.info("tools", "Clearing terrain...")
	...
	Log.info("tools", "Terrain cleared.")
