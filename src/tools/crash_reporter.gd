...
func _ready() -> void:
	...
	Log.info("tools", "CrashReporter logging to: %s" % ProjectSettings.globalize_path(log_path))
...
func connect_to_systems(world_explorer: Node, wsm: Node = null, _os: Node = null, diag: Node = null) -> void:
	...
	Log.info("tools", "CrashReporter connected to systems")
...
func dump_state_now() -> void:
	...
	_write_state()
	Log.info("tools", "CrashReporter state dumped to: %s" % ProjectSettings.globalize_path(log_path))
