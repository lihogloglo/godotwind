...
6. **NEAR-tier refactor in progress (S.0 + S.1 shipped 2026-04-19, Wins 0-5 server-direct/off-thread pass shipped 2026-04-25, branch `perf/distant-rendering-2026-04-17`).** MID / HLOD / IMPOSTORS parked at boot. Per-object tier branching deleted. **Wins 0-5 (2026-04-25):** off-thread cell-collision triangle assembly via WorkerThreadPool (Win 1), server-direct cell static body via PhysicsServer3D RID instead of StaticBody3D (Win 2), distance-sorted collision drain order (Win 3), lazy-spawn lights past 60m with always-spawn override for big lights ≥700 MW radius (Win 4a), server-direct OmniLight3D via RS.omni_light_create + instance_create with flicker/pulse flag-gate keeping animated lights on Node3D path (Win 4b). Authoritative current audit in `docs/audit/godot_46_near_streaming_aaa_audit_2026_04_29_codex.md`; active regression tracker in `docs/audit/near_streaming_regression_tracker_2026_04_29_codex.md`; in-flight plan in `docs/plans/streaming_stutter_2026_04_25.md`. Bench measurement (`--bench-auto=p5_post_near_pass`) + interactive pilot pending. Open issues: (a) `f_terrain_rock_bc_11_nif.res` quarantined as `.crashtest` — rebake pending. (b) Second native SIGSEGV site at sec=~183; new Win 1 worker code adds breadcrumbs (`collision_worker_dispatch`, `collision_finalize_begin/end`) to narrow if it recurs.
