...
6. **NEAR-tier stabilization in progress (branch `perf/distant-rendering-2026-04-17`).** MID / HLOD / IMPOSTORS remain parked at boot while NEAR is stabilized. **State as of 2026-05-01 (committed):** Phase 0A/0B (dead-path deletion + `CellPayload.publish_step()` seam), Phase 1 lane 1 (model callback ownership), Phase 1 lane 2 (collision publish ownership), Phase 2A (transitional renderer-owned RS bucket routing — heavy static refs no longer feed `add_child()`), and P0.4 (lifecycle audit + ownership map + Phase F shape-cache ordering fix + `StaticShapeCache` single-flight + reclaim-after-unload-limbo invariant proven via boomerang route + extended limbo debug hook). Checkpoint commits `1836d97` and `0f6d7d8`. The original 2026-04-29 cache-eviction crash (`ModelLoader._evict_if_over_budget -> _model_cache.erase`) was fixed earlier by `StreamedResourceHandle` ownership; eviction is active again without releasing live handles. **Outstanding:** Phase 2B written design (material/RID lifetime, bucket hide/free order, why-safe-under-unload), Phase 2B implementation (per-cell MultiMesh or canonical batching to replace transitional RS-instance-per-submesh), residual east-route 50ms+ outlier attribution (currently 1 frame >50ms in some runs, attributed to `instantiate`), Phase 3 frame budget split per lane (target 1ms/frame at 150 FPS; currently 8ms 60-FPS-era), and Phase 4 distant rendering re-enable. **Do not re-enable distant rendering** until Phase 2B lands cleanly and 50ms+ outliers are eliminated. Current tracker: `docs/plans/near_streaming_p0_fix_tracker_2026_04_29_codex.md`. Architecture north star: `docs/reference/how to streaming.md`. P0.4 ownership map: `docs/audit/near_streaming_lifecycle_audit_2026_04_30_coder.md`.
