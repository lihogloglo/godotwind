...
| Modular Streaming Ownership | Generic `WorldObjectSource` manifests now feed HLOD, FAR impostors, distant lights, the new world-cell async path, and streaming orchestration. Morrowind/ESM lookup is isolated for provider-driven paths and `ReferenceInstantiator` base-record resolution now goes through the injected spawn adapter. Remaining near/interior legacy routes in `CellManager` still bridge through adapter-local parser-shaped records, and the `ReferenceInstantiator` legacy tail still consumes parser-shaped `CellReference` / ESM records. Canonical toggles are `near_gameplay`, `static_visuals`, `hlod`, `far_impostors`, and `distant_lights`, with temporary aliases for old benchmark names. |
| Framework Boundary | Public world-object legacy escape hatches are removed: no `get_legacy_*`, `WorldObjectRecord.legacy_*`, or `WorldCellManifest.legacy_cell_record` in generic APIs. `CellPayload` and static collision now consume normalized records/transforms, `MorrowindDataProvider` lives under the Morrowind adapter path, distant lights receive generic animation/fire metadata instead of raw MW flag masks, distant-light page/ring math can use the active `WorldCoordinateMapper`, and source-ref base-record lookup from `ReferenceInstantiator` is adapter-owned. Remaining known debt includes `CellManager` `get_source_*` parser bridges, coordinate/distance policy, terrain LAND/LTEX handling, impostor-candidate content policy, weather/dialogue compatibility, and character/NIF assumptions; see `docs/audit/morrowind_framework_boundary_mega_audit_2026_05_21.md`. |
...
| System | Notes |
|--------|-------|
| Ocean/Water | OceanManager, FFT waves, GPU-readback buoyancy, visible FFT ocean surface, depth-derived Beer-Lambert tint, shore mask, custom surface SSR, experimental/off-by-default controlled surface refraction, `WaterSurfaceState`, `UnderwaterCompositorEffect`, retained object wetness, live contact wetness, and receiver waterline diagnostics. Ocean Lab also has a default-off `Shader: Boujie High` surface material; it keeps Godotwind FFT/shore/SSS/foam/spray but uses transparent screen-reading Boujie-style refraction/fog/Fresnel/Snell optics, and it disables the separate controlled surface-refraction layer while active to avoid double refraction. The `Boujie Full` lab preset now exercises the main visual stack together: Boujie surface, FFT-aware `WaterSurfaceState`, live wetness, high underwater medium, underwater particulates, caustics, spray, and SSR. Receiver waterline remains available through `WL Inspect`/`WL Replace` diagnostics, not as a second production underwater-medium path. Retained terrain wetness is not accepted yet; it needs a GPU accumulation mask rather than CPU-side terrain sampling. The 2026-05-23 Ocean Lab controlled-refraction prototype uses a half-resolution water-excluded source, renderer-native source matrices, and a depth-guided receiver ray/depth solve; it remains an exact receiver-waterline tool, not the default open-ocean surface path. World Explorer can force-initialize ocean through tooling, but the full ocean/underwater/wetness stack is not accepted as a default main-scene feature. See `docs/systems/ocean/architecture.md`, `docs/systems/ocean/godot_4_6_water_rendering_rules.md`, and `docs/audit/openmw_style_refraction_source_2026_05_23.md`. |
