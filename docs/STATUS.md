...
| 3-Tier LOD | NEAR 0-150m (Node3D+physics), MID 150-500m (single raw RS instance per object with embedded `ImporterMesh.generate_lods()` cascade, engine-driven screen-space LOD selection via `mesh_lod_threshold`+`lod_bias`), FAR 500-5km (octahedral impostors, v4+v5 dual variant shader, Brucks tri-sample blending in v5). Canonical B-wide rewrite landed in working tree on `refactor/lod-b-wide` 2026-04-10 — see `docs/DISTANCE_RENDERING.md` + `docs/audit/LOD_REFACTOR_B_WIDE.md`. Dead standalone `octahedral_impostor.gdshader` deleted 2026-04-12 (inline shader in `native_impostor_renderer.gd` is the single source). 12/937 impostors rebaked to v5; full v5 rebake pending. Phase G visual + perf validation pending user-piloted pass. Differential impostor updates on cell crossing (~1ms vs 14-158ms before). |
...
| ObjectPaging (runtime HLOD) | OpenMW-style distance-adaptive chunk paging. `src/core/world/object_paging.gd` (renamed from `runtime_hlod_merger.gd` in Phase 6). Phases 1-5 complete as of 2026-04-15: extracted merge kernel to C# (`NativeObjectPagingKernel.cs`), projected-size filter with SizeCache, record-type filter, cost-benefit merge decision, three-tier adaptive walker (1×1 / 2×2 / 4×4 chunks at [150,300) / [300,600) / [600,1000)m bands), hysteresis retention, teleport warmup, minSizeMergeFactor second-pass. 104 unit tests pass. **Disabled by default** (`enabled = false`, console: `hlod_enable`) — runtime perf validation blocked on sig 139 streaming crash (@roaster's scope) and exported-build `ImporterMesh.generate_lods()` worker-thread gate (plan §14 risk 2). Plan + execution log: `docs/audit/OBJECT_PAGING_PLAN.md`, `docs/audit/OBJECT_PAGING_SESSION_2026_04_14_15.md`. |

## Project-wide settings flipped 2026-04-08

- `physics/common/physics_interpolation = true` (Godot 4.4+ canonical pattern, Glenn Fiedler "Fix Your Timestep!" architecture). Per-node carve-outs in `player_controller.gd::camera_pivot` and `fly_camera.gd` set `physics_interpolation_mode = OFF` so mouse-driven rotation stays render-rate fresh. Carry held body also carved out on grab + restored on release. See `docs/INTERACTION_SYSTEM.md` §17.1 for the migration plan + remaining audit (`grep "global_transform = " in _process` callsites).
