...
| Interior Transitions | Classic travel-door path is wired into the main scene. `DoorInteractable` activation routes by placed-door `instance_key` (base record id + ref_num + source cell), so duplicate DOOR records no longer collide. `NativeStreamingManager.set_world_tracking_frozen()` freezes exterior camera tracking while keeping async completions and `CellManager.process_async_instantiation()` alive, so rush-entering a pocket cannot starve the bridge. Exterior→interior, interior→exterior, and interior→interior use the same fade/priority/wait path. Seamless portal/walk-through remains experimental and parked. |
