...
6. **NEAR-tier stabilization in progress (branch `perf/distant-rendering-2026-04-17`).** MID / HLOD / IMPOSTORS remain parked at boot while NEAR is stabilized. The 2026-04-29 cache-eviction crash path (`ModelLoader._evict_if_over_budget -> _model_cache.erase`) is fixed in the P0.2 handle-ownership slice: streamed `PackedScene` resources now use `StreamedResourceHandle`, loaded cells keep strong handles on the `cell_node`, and cache eviction is active again without releasing live handles. Verification on 2026-04-30: 20s dense-loop smoke completed with eviction active; 45s straight east smoke covered 38 fresh cell transitions with no eviction crash; visible `scenes/Godotwind.tscn` manual traversal worked well and user quit cleanly. Hidden automated quit probes can still hit a post-`LOG_EXIT` native finalization signal 11, but that did not reproduce on the visible manual quit. Current tracker: `docs/plans/near_streaming_p0_fix_tracker_2026_04_29_codex.md`. Recovery handoff / architecture guide: `docs/reference/how to streaming.md`. Next architectural phase is P0.3 per-payload publish scheduling; do not resume distant rendering before checkpointing/reviewing this stable slice.
