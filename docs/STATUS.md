...
| System | Notes |
|--------|-------|
| Ocean/Water | OceanManager, FFT waves, GPU-readback buoyancy, visible FFT ocean surface, depth-derived Beer-Lambert tint, shore mask, custom surface SSR/refraction, `WaterSurfaceState`, `UnderwaterCompositorEffect`, retained object wetness, and an off-by-default receiver-waterline diagnostic stack. Current source still has the surface shader sampling screen/depth textures for surface effects; receiver waterline is quarantined until accepted by a dedicated plan and budget. World Explorer can force-initialize ocean through tooling, but the full ocean/underwater/wetness stack is not accepted as a default main-scene feature. See `docs/systems/ocean/architecture.md` and `docs/systems/ocean/godot_4_6_water_rendering_rules.md`. |
