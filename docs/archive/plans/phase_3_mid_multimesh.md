...
5. ✅ Port cull pass to C# (`WorldMidCuller.cs`). Returns `{visible: int, buffer: PackedFloat32Array}` Dictionary. Frustum cull deferred — engine AABB on the RS instance handles whole-batch frustum at world scale. Registry lazily instantiates via NativeBridge on first tick, passes to each batch's `cull_and_upload(cam_pos, max_dist_sq, native_culler)`. (commit `b35cd1c`)
6. ✅ Rewrite `lod_crossfade_multimesh.gdshader` for spawn-time fade via `INSTANCE_CUSTOM.x`/`.y`. Mirrors Phase 2's `lod_crossfade.gdshader` formula, fed per-slot. PrototypeBatch._init lazily builds a ShaderMaterial from the prototype's StandardMaterial3D (copies albedo_texture/color/roughness/metallic/specular + alpha-scissor flags) — one ShaderMaterial per batch; per-slot timing via INSTANCE_CUSTOM. (commit `1291655`)
7. ✅ Deleted `batch_cell_into_multimesh`, `_create_cell_batch`, `_hide_batch_slot`, `_show_batch_slot`, `_free_cell_batches`, `CellBatch` inner class, `_cell_batches` dict, `MM_BATCH_MIN_COUNT` const, `InstanceData.mm_slot` + `.batch` fields, `use_prototype_registry` feature flag, `_sub_local` helper. cell_manager._finalize_request simplified. world_explorer `proto_registry` command reduced to status-only. Registry is now the only MID-tier path. Legacy per-sub-mesh RS path retained ONLY as the register_mesh_type direct-registration fallback (tests / debug tools). (commit TBD — this step)
8. ✅ Commit per step. Branch isolated. 7 commits: d71302c, f5472b1, 5e2450a, 93973df, f3105ca, b35cd1c, 1291655, + step-7 commit.
